...
def parse_args():
    ...
    parser.add_argument("--ckpt", type=str, default='_checkpoint/')
...
if __name__ == '__main__':
    ...
    '''
    checkpoints_path = cfg.name + cfg.ckpt
    output_path = cfg.name + cfg.output

    directory_ckp = checkpoints_path+"%s_ckp" % (networkname)
    if not os.path.exists(directory_ckp):
        os.makedirs(directory_ckp)

    if not os.path.exists(output_path):
        os.makedirs(output_path)
    directory_t = output_path+'%s' % (networkname)
    if not os.path.exists(directory_t):
        os.makedirs(directory_t)

    txt_name = checkpoints_path+'lf_%s.txt' % (networkname)
