...
from tensorflow.contrib.keras.api.keras.layers import Dropout, BatchNormalization, GlobalAveragePooling2D
...
def my_channel_attention(cost_volume):
    x = GlobalAveragePooling2D()(cost_volume)
    x = Lambda(lambda y: K.expand_dims(K.expand_dims(y, 1), 1))(x)
    x = Conv2D(170, 1, 1, 'same')(x)
    x = Activation('relu')(x)
    x = Conv2D(15, 1, 1, 'same')(x) # [B, 1, 1, 1, 15]
    ...
    attention = Lambda(lambda y: K.reshape(y, (K.shape(y)[0], 1, 1, 81)))(x)
...
def basic(sz_input, sz_input2):
    ...
    cost = Conv2D(num_cls, 3, strides=1, padding='same', use_bias=False)(classify)

    return Model(inputs=i, outputs=[cost])

def disparityregression(input):
    shape = K.shape(input)
    disparity_values = np.linspace(-4, 4, num_cls)
...
def NET(sz_input, sz_input2, learning_rate, train=True):
    ...
    """ feature volume """
    cv = Lambda(lambda x:  K.concatenate(x, axis=3))(feature_list)

    """ channel attention """
    cv, attention = my_channel_attention(cv)
